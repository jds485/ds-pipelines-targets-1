...
plotDiagnostics <- function(data,
  ...
  #number of profiles
  nProfs <- sort(unique(data$n_prof))
  ...
  axis(1, at = c(-100, nProfs, 1e10), labels = c("", nProfs, ""), tck = -0.01)
  axis(2, at = seq(0,10), las = 1, tck = -0.01)
  
  #Plot data
  for (mod in c('pb','dl','pgdl')){
    modData <- filter(data, model_type == mod)
    modProfiles <- unique(modData$n_prof)
    for (mod_profile in modProfiles){
      d <- filter(modData, n_prof == mod_profile) %>% summarize(y0 = min(rmse), y1 = max(rmse), col = unique(col))
      xPos <- offsets %>% filter(n_prof == mod_profile) %>% pull(!!mod) + mod_profile
      lines(c(xPos, xPos), c(d$y0, d$y1), col = d$col, lwd = 2.5)
    }
    d <- group_by(modData, n_prof) %>% summarize(y = mean(rmse), col = unique(col), pch = unique(pch)) %>%
